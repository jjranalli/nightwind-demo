...
export default function Home() {
	...
	useEffect(() => {
		function randomPath() {
			...
			const c2_2down =
				Math.sqrt(
					Math.pow(width / 2, 2) - Math.pow(width - Number(c2_1), 2)
				) + Number(heightEnd)
			const c2_2up = c2_2down - (c2_2down - height / 2) * 2
			const c2_2 = randomBoolean2
				? c2_2down.toFixed(2)
				: c2_2up.toFixed(2)
			...
			<main className="text-center">
							...
							{paths.map((path, i) => (
								<div
									...
									className={`text-sm py-1 bg-white rounded-md m-2 border-2 ${
										...
										.join(
											"-"
										)} ring-${path.randomClass
										.split("-")
										.splice(1, 2)
										.join(
											"-"
										)} ring-2 ring-opacity-30 hover:ring-opacity-60`}
								>
									<p>
										{path.randomClass
											.split("-")
				...
				<div className="px-4">
					<div className="nightcard">
						...
						<p>
							<span>
								and still compatible by default with nightwind
							</span>
						</p>
					</div>
				</div>
